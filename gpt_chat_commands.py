...
async def handle_message(message: types.Message, bot):
    ...
    try:
        ...
        if message_count >= 7:
            message_count = 0
            await message.reply("Если хотите помочь переходу на ГПТ 4 и качественный хостинг\n https://www.tinkoff.ru/cf/ANd16yXFVM7 .")
...
def setup_gpt_chat_handlers(dp, bot):
    ...
    dp.message_handler(content_types=['text'], chat_type=types.ChatType.PRIVATE)(lambda message: handle_message(message, bot))
